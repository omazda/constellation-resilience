...
**Демо: [braining.space](https://braining.space)** · **Презентация: [`presentation Final.pdf`](presentation%20Final.pdf)** в корне репозитория
...
| Документ | О чём |
|---|---|
| [`presentation Final.pdf`](presentation%20Final.pdf) | **презентация решения: подход, сравнение конфигураций, поведение при отказах, рекомендации** |
...
синтаксис GitHub Actions и читает оба каталога. На обеих площадках проверяется сходимость расчёта
с эталоном по четырём сценариям и 73 теста.

Сборка проверяется по-разному, и это вынужденно: на облачных раннерах GitVerse нет доступа к
`docker.sock`, поэтому `docker build` там невозможен. На GitHub собирается сам образ и внутри него
поднимается сервис; на GitVerse те же две стадии выполняются нативно — `nuxt generate`, затем
`uvicorn` с собранной статикой, и проверяется, что отвечают `/health`, корень и клиентский
маршрут через SPA-fallback.
