...
- **Кеш расчёта — в памяти процесса** и пересчитывается после перезапуска. Сами варианты
  конфигурации при этом хранятся на диске (`/app/data/variants`, именованный том в compose) в
  формате `cosmo-A-1.0` и переживают пересоздание контейнера; список отдаётся сообщением
  `variants.list`. Кеш на диск не кладём намеренно: он выводится из сценария за секунду, а весит
  сотни килобайт.
