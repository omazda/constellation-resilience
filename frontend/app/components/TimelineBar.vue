 ...
 * конкретного сценария, только про переданный массив секунд, см. красные флаги в CLAUDE.md про
 * хардкод «720 отсчётов»).
 *
 ...
 * бинарного пакета `compute` (см. `.claude/rules/protocol.md`) держат одну и ту же ось без
 * пересчёта секунд туда-обратно.
 *
 * Собран из готовых компонентов Nuxt UI (.claude/rules/frontend.md): `USlider` — сам скраб,
