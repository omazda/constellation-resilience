...
// см. docs/ARCHITECTURE.md, Dockerfile) и SPA-fallback на `200.html`. Серверный рендер тут не
...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    public: {
      ...
      // поднят на 3000, а бэкенд отдельно на 8000 (см. docs/PROTOCOL.md, CORS разрешён
      // именно для этого случая). Пусто -> берём текущий origin (прод-сборка в одном контейнере,
      // docs/ARCHITECTURE.md). Переопределяется переменной окружения `NUXT_PUBLIC_WS_URL`.
      wsUrl: '',
